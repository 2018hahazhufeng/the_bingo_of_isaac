...
function Bingo:gameStartMenu()
    if Bingo.gameIsStarted==false then
        ...
        Bingo.startMenu:DrawStringUTF8(Bingo.nameAndVersion,Bingo.renderPosition.X-Bingo.FONT_OFFSET*8,Bingo.renderPosition.Y-Bingo.FONT_OFFSET*2+5,KColor(1,1,1,1))
        ...
        if Bingo.meunIsChanged==false then
            ...
            Bingo.startMenu:DrawStringUTF8("按↑、↓键切换选择，按使用主动键确认",Bingo.renderPosition.X-Bingo.FONT_OFFSET*9,Bingo.renderPosition.Y-Bingo.FONT_OFFSET*3,KColor(1,1,1,1))
        ...
        if Bingo.meunIsChanged then
            ...
            elseif Bingo.startMenuRootSelect==2 then
                ...
                Bingo.startMenu:DrawStringUTF8(Bingo.keyboard.getRoomIdText(),Bingo.renderPosition.X-Bingo.FONT_OFFSET*9,Bingo.renderPosition.Y+Bingo.FONT_OFFSET*3,KColor(1,1,1,1))
                ...
                if Bingo.enableCooperatedMode then
                    Bingo.startMenu:DrawStringUTF8("按地图键选择是否是组队模式",Bingo.renderPosition.X-Bingo.FONT_OFFSET*20,Bingo.renderPosition.Y+Bingo.FONT_OFFSET*6,KColor(0,255,0,1))
                else
                    Bingo.startMenu:DrawStringUTF8("按地图键选择是否是组队模式",Bingo.renderPosition.X-Bingo.FONT_OFFSET*20,Bingo.renderPosition.Y+Bingo.FONT_OFFSET*6,KColor(1,1,1,1))
                end
                if wsConnectStatus then
                    Bingo.startMenu:DrawStringUTF8("连接成功，按主动键断开连接",Bingo.renderPosition.X-Bingo.FONT_OFFSET*15,Bingo.renderPosition.Y+Bingo.FONT_OFFSET*4+5,KColor(1,1,1,0.7))
                ...
                if Input.IsActionTriggered(ButtonAction.ACTION_ITEM,Bingo.player.ControllerIndex) and IsaacSocket then
                    ...
                    local partsSeed=split(Bingo.seedForShow," ")
                    Bingo.ws = IsaacSocket.WebSocketClient.New("wss://pbkyopvordqk.sealoshzh.site:443/ws?type=&userId="..Bingo.userId.."&roomId="..roomIdParts[1].."&seed="..partsSeed[1]..partsSeed[2].."&seatIndex="..roomIdParts[2], CallbackOnOpen, CallbackOnMessage, CallbackOnClose, CallbackOnError)
                end
            --daily-run mode--
            else
                --daily-run mode--
...
function Bingo:showGameOverInfo()
    if Bingo.gameIsOver~=0 then
        ...
        Bingo.startMenu:DrawStringUTF8("按R键重玩这局",Bingo.renderPosition.X-Bingo.FONT_OFFSET*4-2,Bingo.renderPosition.Y+Bingo.FONT_OFFSET*7,KColor(1,1,1,1))
...
function Bingo:tasksIconRender()
    if Bingo.gameIsStarted==true then
        ...
        if Bingo.taskSelectionEnable then
            ...
            Bingo.taskSelection:Render(Vector(Bingo.renderPositionOfTasks.X+Bingo.taskSelectionPosition.X*26+1,Bingo.renderPositionOfTasks.Y+Bingo.taskSelectionPosition.Y*26+1))
...
function Bingo:killFinalBosses()
    ...
    if Bingo.finalBossPtr.ref~=nil and Bingo.finalBossPtr.ref.Ref~=nil and Bingo.finalBossPtr.ref.Ref:IsDead() then
        if Bingo.finalBossPtr.type==EntityType.ENTITY_BEAST or Bingo.finalBossPtr.type==EntityType.ENTITY_MEGA_SATAN_2 then
            Isaac.Spawn(Isaac.GetEntityTypeByName("stopEndAnimation"), 0, 0, Isaac.GetFreeNearPosition(Bingo.player.Position, 3), Vector(0,0),Bingo.player)
            Isaac.Spawn(5, 340, 0, Vector(0,0), Vector(0,0), nil)
        end
        Isaac.Spawn(5,100,Bingo.restartKey,Isaac.GetFreeNearPosition(Bingo.player.Position,3),Vector(0,0),Bingo.player)
